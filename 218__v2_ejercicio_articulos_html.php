<?php
	...
	if (isset($_COOKIE['articulos'])) {
		$compra= $_COOKIE['articulos'];
		$tablaCompra=json_decode($compra,true);
		//print_r($tablaCompra);
	} else {
		//crear cookie
		setcookie('articulos', null, time()+3600*24*365);
		//Crear array de sesion ($_SESSION)
		if (isset($_SESSION['articulos'])) {
			$tablaCompra = $_SESSION['articulos'];
		} else {
			$tablaCompra = array();
		}
	}
	...
	//actualiza la cookie cestaCompra
	$listaContenido=json_encode($tablaCompra);
	setcookie('articulos',$listaContenido, time()+3600*24*365);
